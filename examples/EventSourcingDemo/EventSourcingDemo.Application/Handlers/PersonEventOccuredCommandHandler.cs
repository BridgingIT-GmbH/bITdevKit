...
public sealed class PersonEventOccuredCommandHandler :
    CommandHandlerBase<AggregateEventOccuredCommand<Person>, bool> // <1>
{
    private readonly IPersonOverviewRepository personOverviewRepository;
    private readonly IEntityMapper mapper;

    public PersonEventOccuredCommandHandler(ILoggerFactory loggerFactory, IPersonOverviewRepository personOverviewRepository, IEntityMapper mapper)
        : base(loggerFactory)
    {
    ...
    public override Task<CommandResponse<bool>> Process(AggregateEventOccuredCommand<Person> request, CancellationToken cancellationToken)
    {
        if (request is null)
        {
        ...
        System.Diagnostics.Debug.WriteLine("Do Event propagation for " + request.Aggregate.Id);
