    ...
    public async Task DispatchAndWaitAsync_StaleCity_PersistsWeatherData()
    {
        ...
        using var harness = JobSchedulerTestHarness.Create()
            ...
            .WithServices(services =>
            {
                ...
                services.AddBusinessCalendars(calendars => calendars.SetDefault(new BusinessCalendar()));
        ...
        await queueBroker.Received(1).Enqueue(
            ...
            Arg.Any<CancellationToken>());
        await queueBroker.Received(1).Enqueue(
            Arg.Is<WeatherReportGenerationMessage>(m => m.GetCityId() == city.Id && m.ReportType == WeatherReportType.NextBusinessDay),
            Arg.Any<CancellationToken>());
    }
}
