...
public sealed class ResolveWeatherReportPeriodsStepTests
{
    ...
    [Fact]
    public async Task Execute_CustomBusinessCalendar_ResolvesNextBusinessDayReportPeriod()
    {
        // Arrange
        var calendar = new BusinessCalendar(nonWorkingDays: [DayOfWeek.Saturday, DayOfWeek.Sunday, DayOfWeek.Monday]);
        var sut = new ResolveWeatherReportPeriodsStep(
            NullLogger<ResolveWeatherReportPeriodsStep>.Instance,
            new FixedTimeProvider(new DateTimeOffset(2026, 06, 26, 10, 00, 00, TimeSpan.Zero)),
            new TestBusinessCalendarResolver(calendar));
        var context = new WeatherIngestionContext(CreateCity("Europe/London"));
        ...
        // Assert
        control.Result.IsSuccess.ShouldBeTrue();
        context.NextBusinessDayReportPeriod.ForecastDateStart.ShouldBe(new DateOnly(2026, 06, 30));
        context.NextBusinessDayReportPeriod.ForecastDateEndExclusive.ShouldBe(new DateOnly(2026, 07, 01));
        context.NextBusinessDayReportPeriod.PeriodStartUtc.ShouldBe(new DateTime(2026, 06, 29, 23, 00, 00, DateTimeKind.Utc));
        context.NextBusinessDayReportPeriod.PeriodEndUtc.ShouldBe(new DateTime(2026, 06, 30, 23, 00, 00, DateTimeKind.Utc));
    }
    ...
    private sealed class TestBusinessCalendarResolver(IBusinessCalendar calendar) : IBusinessCalendarResolver
    {
        public IBusinessCalendar Resolve(CultureInfo culture = null) => calendar;

        public IBusinessCalendar Resolve(string code) => calendar;
    }
}
