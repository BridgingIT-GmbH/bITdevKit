        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!instance.Options.BackoffExponential)
        {
            retryPolicy = Policy.Handle<Exception>().WaitAndRetryForeverAsync(
                ...
                onRetry: (ex, wait) =>
                {
                    this.Logger.LogError(ex, "{LogKey} command circuitbreaker behavior (attempt=#{Attempts}, wait={Wait}, type={BehaviorType}) {ErrorMessage}", Constants.LogKey, attempts, wait.Humanize(), this.GetType().Name, ex.Message);
                    attempts++;
                });
        }
        else
        ...
        var circuitBreakerPolicy = Policy
            ...
            .CircuitBreakerAsync(
                ...
                onBreak: (ex, wait) =>
                {
                    this.Logger.LogError(ex, "{LogKey} command circuitbreaker behavior (circuit=open, wait={Wait}, type={BehaviorType}) {ErrorMessage}", Constants.LogKey, wait.Humanize(), this.GetType().Name, ex.Message);
                },
                onReset: () => this.Logger.LogDebug("{LogKey} command circuitbreaker behavior (circuit=closed, type={BehaviorType})", Constants.LogKey, this.GetType().Name),
                onHalfOpen: () => this.Logger.LogDebug("{LogKey} command circuitbreaker behavior (circuit=halfopen, type={BehaviorType})", Constants.LogKey, this.GetType().Name));
