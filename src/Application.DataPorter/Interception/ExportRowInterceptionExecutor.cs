        ...
        foreach (var interceptor in this.interceptors)
        {
            ...
            if (decision.Outcome == RowInterceptionOutcome.Skip)
            {
                this.logger.LogWarning(
                    "Export row interceptor {InterceptorType} skipped row {RowNumber} in {Format}{Sheet} ({Reason})",
            ...
            {
                this.logger.LogWarning(
                    "Export row interceptor {InterceptorType} aborted row {RowNumber} in {Format}{Sheet} ({Reason})",
                    interceptor.GetType().Name,
                    rowNumber,
                    format,
                    string.IsNullOrWhiteSpace(sheetName) ? string.Empty : $" [{sheetName}]",
