...
public class InProcessMessageBroker : MessageBrokerBase
{
    ...
    public InProcessMessageBroker(Builder<InProcessMessageBrokerOptionsBuilder, InProcessMessageBrokerOptions> optionsBuilder)
    ...
    protected override Task OnPublish(IMessage message, CancellationToken cancellationToken)
    {
        ...
        this.messageProcessor.Post(
            new MessageRequest(message, result => tcs.SetResult(result), cancellationToken)); // TODO: message.Clone(), has issues with inheritance (EchoMessage = Message after clone)
        return tcs.Task;
    }
