...
public static partial class ResultTTaskExtensions
{
    ...
    /// <param name="binder">An asynchronous function that transforms the success value.</param>
    /// <returns>
        ...
        Func<T, Task<T>> binder)
    {
        ...
        try
        {
            var newValue = await binder(result.Value).AnyContext();
    ...
    public static async Task<Result<T>> BindAsync<T>(
        ...
        Func<T, CancellationToken, Task<T>> binder,
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var newValue = await binder(result.Value, cancellationToken)
                .AnyContext();
    ...
    /// <typeparam name="T">The type of the encapsulated success value.</typeparam>
    /// <param name="task">An awaited <see cref="Task"/> returning a <see cref="Result{T}"/>.</param>
    /// <param name="binder">A function that transforms the success value.</param>
    /// <returns>
    /// A task returning a new <see cref="Result{T}"/> containing the transformed value,
    ...
    public static async Task<Result<T>> BindResultAsync<T, TInner>(
        this Result<T> result,
        Func<T, CancellationToken, Task<Result<TInner>>> binder,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var inner = await binder(result.Value, cancellationToken)
                .AnyContext();
    ...
    /// <param name="binder">Async operation returning a <see cref="Result{TInner}"/>.</param>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return await result
            .BindResultAsync(binder, merge, cancellationToken).AnyContext();
    }
    ...
    /// </code>
    /// </example>
    public static async Task<Result<T>> When<T>(
        this Task<Result<T>> resultTask,
        bool condition,
        Func<Result<T>, Result<T>> operation)
    {
        if (!condition || operation is null)
        {
        ...
        {
            var result = await resultTask;
            return result.When(condition, operation);
        }
        catch (Exception ex)
        {
            return Result<T>.Failure()
    ...
    /// <summary>
    ///     Conditionally executes a chain of operations on the Result task based on a predicate evaluated against the value.
    ...
    public static async Task<Result<T>> When<T>(
        ...
        Func<Result<T>, Result<T>> operation)
    {
        if (predicate is null || operation is null)
        {
            return await resultTask;
        }
        ...
        {
            var result = await resultTask;
            return result.When(predicate, operation);
        }
        catch (Exception ex)
        {
            return Result<T>.Failure()
    ...
    ///         .EnsureAsync(async (u, ct2) => await IsActiveAsync(u, ct2), new Error("Must be active"), ct)
    ///         .BindAsync(async (u, ct2) => await EnrichAdminDataAsync(u, ct2), ct)
    ///     , cancellationToken);
    /// </code>
    /// </example>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            var result = await resultTask;
            return await result.WhenAsync(condition, operation, cancellationToken);
        }
        catch (OperationCanceledException)
        {
            return Result<T>.Failure()
    ...
    /// <summary>
    ///     Asynchronously and conditionally executes a chain of operations on the Result task based on a predicate evaluated against the value.
    ///     If the predicate returns false, the original result is returned unchanged.
    /// </summary>
    /// <typeparam name="T">The type of the result value.</typeparam>
    /// <param name="resultTask">The Result task to operate on.</param>
    ...
    ///         o => o.Total > 1000,
    ///         async (r, ct) => await r
    ///             .BindAsync(async (o, ct2) => await ApplyVipDiscountAsync(o, ct2), ct)
    ///             .TapAsync(async (o, ct2) => await NotifyAccountManagerAsync(o, ct2), ct),
    ///         cancellationToken
    ///     );
    /// </code>
    /// </example>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            var result = await resultTask;
            return await result.WhenAsync(predicate, operation, cancellationToken);
        }
        catch (OperationCanceledException)
        {
            return Result<T>.Failure()
    ...
    /// produced from the awaited result via <paramref name="argsFactory"/>.
    /// </summary>
    /// <typeparam name="T">The type of the success value contained in the result.</typeparam>
    /// <param name="resultTask">The task returning a result to log.</param>
    ...
    /// A function that receives the awaited result and returns the structured logging arguments.
    /// </param>
    ...
    /// <param name="successLevel">Log level used when the awaited result indicates success.</param>
    /// <param name="failureLevel">Log level used when the awaited result indicates failure.</param>
    /// <returns>The awaited result, unchanged.</returns>
    /// <example>
    /// <code>
    /// await pipeline.Log(
