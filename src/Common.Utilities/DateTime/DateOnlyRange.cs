...
/// <summary>
/// Represents a half-open <see cref="DateOnly"/> range using [start, end) semantics, with one optional open boundary.
/// </summary>
/// <remarks><example><code>var range = new DateOnlyRange(start, end);</code></example></remarks>
public readonly record struct DateOnlyRange : IComparable<DateOnlyRange>, IComparable
{
    /// <summary>
    /// Initializes a new half-open <see cref="DateOnly"/> range with one optional open boundary.
    /// </summary>
    ...
    public DateOnlyRange(DateOnly? startInclusive, DateOnly? endExclusive)
    {
        if (startInclusive is null && endExclusive is null)
        {
            throw new ArgumentException("A DateOnlyRange must have a start or an end boundary.", nameof(startInclusive));
        }
    ...
    /// <summary>Gets the number of days in the range.</summary>
    /// <exception cref="InvalidOperationException">Thrown when the range has an open boundary.</exception>
    public int Days => this.StartInclusive.HasValue && this.EndExclusive.HasValue
        ? this.EndExclusive.Value.DayNumber - this.StartInclusive.Value.DayNumber
        : throw new InvalidOperationException("Open-ended DateOnlyRange values do not have a finite day count.");

    /// <summary>Gets the range duration.</summary>
    public TimeSpan Duration => TimeSpan.FromDays(this.Days);
    ...
    /// <exception cref="ArgumentException">Thrown when <paramref name="obj"/> is not a <see cref="DateOnlyRange"/>.</exception>
    public int CompareTo(object obj)
    {
        return obj is null
            ? 1
            : obj is DateOnlyRange other
                ? this.CompareTo(other)
                : throw new ArgumentException("Object must be a DateOnlyRange.", nameof(obj));
    }
    ...
    private static bool IsStartBeforeEnd(DateOnly? start, DateOnly? end)
    {
        return !start.HasValue || !end.HasValue || start.Value < end.Value;
    }

    private static int CompareStarts(DateOnly? left, DateOnly? right)
    {
    ...
    private static int CompareEnds(DateOnly? left, DateOnly? right)
    {
        return (left.HasValue, right.HasValue) switch
        {
            (false, false) => 0,
    ...
    private void EnsureUsable()
    {
        ...
        {
            throw new InvalidOperationException("A DateOnlyRange must have a start or an end boundary.");
        }
    }
}
