...
/// <summary>
/// Represents a half-open <see cref="DateTimeOffset"/> range using [start, end) semantics, with one optional open boundary.
/// </summary>
/// <remarks><example><code>var range = new DateTimeOffsetRange(start, end);</code></example></remarks>
public readonly record struct DateTimeOffsetRange : IComparable<DateTimeOffsetRange>, IComparable
{
    /// <summary>
    /// Initializes a new half-open <see cref="DateTimeOffset"/> range with one optional open boundary.
    /// </summary>
    ...
    public DateTimeOffsetRange(DateTimeOffset? startInclusive, DateTimeOffset? endExclusive)
    {
        if (startInclusive is null && endExclusive is null)
        {
            throw new ArgumentException("A DateTimeOffsetRange must have a start or an end boundary.", nameof(startInclusive));
        }
    ...
    public TimeSpan Duration => this.StartInclusive.HasValue && this.EndExclusive.HasValue
        ...
        : throw new InvalidOperationException("Open-ended DateTimeOffsetRange values do not have a finite duration.");
    ...
    /// <exception cref="ArgumentException">Thrown when <paramref name="obj"/> is not a <see cref="DateTimeOffsetRange"/>.</exception>
    public int CompareTo(object obj)
    {
        return obj is null
            ? 1
            : obj is DateTimeOffsetRange other
                ? this.CompareTo(other)
                : throw new ArgumentException("Object must be a DateTimeOffsetRange.", nameof(obj));
    }

    /// <summary>Compares whether one range sorts before another range.</summary>
    public static bool operator <(DateTimeOffsetRange left, DateTimeOffsetRange right) => left.CompareTo(right) < 0;
    ...
    private static bool IsStartBeforeEnd(DateTimeOffset? start, DateTimeOffset? end)
    {
        return !start.HasValue || !end.HasValue || start.Value < end.Value;
    }

    private static int CompareStarts(DateTimeOffset? left, DateTimeOffset? right)
    {
    ...
    private static int CompareEnds(DateTimeOffset? left, DateTimeOffset? right)
    {
        return (left.HasValue, right.HasValue) switch
        {
            (false, false) => 0,
    ...
    private void EnsureUsable()
    {
        ...
        {
            throw new InvalidOperationException("A DateTimeOffsetRange must have a start or an end boundary.");
        }
    }
}
