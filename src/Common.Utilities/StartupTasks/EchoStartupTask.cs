...
public class EchoStartupTask : IStartupTask, IChaosExceptionStartupTask, IRetryStartupTask, ITimeoutStartupTask
{
    private readonly ILogger<EchoStartupTask> logger;

    public EchoStartupTask(ILoggerFactory loggerFactory)
    {
        this.logger = loggerFactory?.CreateLogger<EchoStartupTask>() ?? NullLoggerFactory.Instance.CreateLogger<EchoStartupTask>();
    }
