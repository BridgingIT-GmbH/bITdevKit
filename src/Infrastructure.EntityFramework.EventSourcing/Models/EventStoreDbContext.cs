...
public class EventStoreDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<EventStoreAggregateEvent>().ToTable("AggregateEvent").HasIndex(ae => new { ae.AggregateId, ae.AggregateVersion }).HasDatabaseName("IX_AggregateEvent_AggregateIDVersion");
        modelBuilder.Entity<EventStoreAggregateEvent>().HasIndex(a => a.AggregateType)
            .HasDatabaseName("IX_AggregateEvent_AggregateType");
        modelBuilder.Entity<EventStoreAggregateEvent>().HasIndex(a => a.AggregateId)
            .HasDatabaseName("IX_AggregateEvent_AggregateId");
        modelBuilder.Entity<EventStoreAggregateEvent>().HasIndex(a => new { a.AggregateId, a.AggregateType, a.AggregateVersion }).HasDatabaseName("IX_AggregateEvent_IX_AggregateEvent_AggregateIDAggrTypeVers").IsUnique();
        modelBuilder.Entity<Outbox>().ToTable("EventstoreOutbox").HasIndex(e => e.TimeStamp).HasDatabaseName("IX_EventstoreOutbox_TimeStamp");
        modelBuilder.Entity<EventStoreSnapshot>().ToTable("Snapshot").HasIndex(s => new { s.Id, s.AggregateType }).IsUnique();
    }
}