    ...
    where TContext : DbContext, IOutboxDomainEventContext
{
        ...
        OutboxDomainEventOptions options = null)
    {
        ...
        this.logger = loggerFactory?.CreateLogger<OutboxDomainEventWorker<TContext>>() ??
            NullLoggerFactory.Instance.CreateLogger<OutboxDomainEventWorker<TContext>>();
    ...
    public async Task ProcessAsync(string eventId = null, CancellationToken cancellationToken = default)
    {
        ...
        var scopedPublisher =
            new MediatorDomainEventPublisher(this.loggerFactory, scope.ServiceProvider.GetRequiredService<IMediator>());
        var count = 0;
       ...
       // this.logger.LogDebug("++++ OUTBOX: READ DOMAINEVENTS (eventId={EventId})", eventId);
        ...
        await (await context.OutboxDomainEvents.Where(e => e.ProcessedDate == null)
                .WhereExpressionIf(e => e.EventId == eventId,
                    !string.IsNullOrEmpty(eventId)) // OutboxDomainEventProcessMode.Immediate
    ...
    {
        var attempts = (outboxEvent.Properties?.GetValue(OutboxDomainEventPropertyConstants.ProcessAttemptsKey)
                ?.ToString()
                .To<int>() ??
            0) +
            1;
        if (attempts > this.options.RetryCount)
        {
            this.logger.LogWarning(
                "{LogKey} outbox domain event processing skipped: max attempts reached (eventId={DomainEventId}, eventType={DomainEventType}, attempts={DomainEventAttempts})",
                Constants.LogKey,
                outboxEvent.EventId,
                outboxEvent.Type.Split(',')[0],
                attempts - 1);

            try
            {
                var existingMessage = outboxEvent.Properties
                    ?.GetValue(OutboxDomainEventPropertyConstants.ProcessMessageKey)
                    ?.ToString();
                ...
                outboxEvent.Properties.AddOrUpdate(OutboxDomainEventPropertyConstants.ProcessMessageKey,
                    $"max attempts reached (eventId={outboxEvent.EventId}, eventType={outboxEvent.Type.Split(',')[0]}, attempts={attempts - 1}) {existingMessage}");
            ...
            catch (Exception ex)
            {
                this.logger.LogError(ex,
                    "{LogKey} outbox domain event storage update failed: {ErrorMessage} (eventId={DomainEventId}, eventType={DomainEventType})",
                    Constants.LogKey,
                    ex.Message,
                    outboxEvent.EventId,
                    outboxEvent.Type.Split(',')[0]);
        ...
        try
        {
            ...
            if (type is null)
            {
                TypedLogger.LogEventTypeNotResolved(this.logger,
                    "DOM",
                    outboxEvent.EventId,
                    outboxEvent.Type.Split(',')[0]);
                ...
                try
                {
                    ...
                    outboxEvent.Properties.AddOrUpdate(OutboxDomainEventPropertyConstants.ProcessStatusKey, "Failure");
                    outboxEvent.Properties.AddOrUpdate(OutboxDomainEventPropertyConstants.ProcessMessageKey,
                        $"event type could not be resolved (eventId={outboxEvent.EventId}, eventType={outboxEvent.Type.Split(',')[0]})");
                        ...
                        .StartActvity($"OUTBOX_PROCESS {eventType}",
                            async (a, c) =>
                            {
                               ...
                               // this.logger.LogDebug("++++ WORKER: PROCESS STORED DOMAINEVENT {@DomainEvent}", @event);
                                ...
                                outboxEvent.ProcessedDate ??=
                                    DateTime.UtcNow; // attempt successfull, don't process again
                                outboxEvent.Properties.AddOrUpdate(OutboxDomainEventPropertyConstants.ProcessStatusKey,
                                    "Success");
                                outboxEvent.Properties.AddOrUpdate(OutboxDomainEventPropertyConstants.ProcessMessageKey,
                                    string.Empty);
                                outboxEvent.Properties.AddOrUpdate(
                                    OutboxDomainEventPropertyConstants.ProcessAttemptsKey,
                                    attempts);
                                await context.SaveChangesAsync<OutboxDomainEvent>(c)
                                    .AnyContext(); // only save changes in this scoped context
                            },
        ...
        {
            this.logger.LogError(ex,
                "{LogKey} outbox domain event processing failed: {ErrorMessage} (eventId={DomainEventId}, eventType={DomainEventType}, attempts={DomainEventAttempts})",
                Constants.LogKey,
                ex.Message,
                outboxEvent.EventId,
                outboxEvent.Type.Split(',')[0],
            ...
            {
                this.logger.LogError(ex,
                    "{LogKey} outbox domain event storage update failed: {ErrorMessage} (eventId={DomainEventId}, eventType={DomainEventType}) {ErrorMessage}",
                    Constants.LogKey,
                    ex.Message,
                    outboxEvent.EventId,
                    outboxEvent.Type.Split(',')[0],
    ...
    public static partial class TypedLogger
    {
        [LoggerMessage(0,
            ...
            "{LogKey} outbox domain events processing (context={DbContextType}, eventId={DomainEventId})")]
        ...
        [LoggerMessage(1,
            LogLevel.Information,
            "{LogKey} outbox domain events processed (context={DbContextType}, count={outboxDomainEventProcessedCount})")]
        public static partial void LogProcessed(
            ILogger logger,
            string logKey,
            string dbContextType,
            int outboxDomainEventProcessedCount);
        ...
        [LoggerMessage(3,
            LogLevel.Error,
            "{LogKey} outbox domain event type could not be resolved (eventId={DomainEventId}, eventType={DomainEventType})")]
        public static partial void LogEventTypeNotResolved(
            ILogger logger,
            string logKey,
            string domainEventId,
