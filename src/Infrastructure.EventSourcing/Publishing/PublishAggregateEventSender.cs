    ...
    IAggregateEventOutboxSender aggregateEventOutboxSender) : IPublishAggregateEventSender
{
    ...
    /// Wenn in <see cref="eventStorePublishingModes"/> das Flag <see cref="EventStorePublishingModes.AddToOutbox"/> gesetzt ist,
    ...
    {
        if ((this.eventStorePublishingModes &
             EventStorePublishingModes.AddToOutbox) != EventStorePublishingModes.None)
        {
    ...
    /// <summary>
    /// Veröffentlicht das DomänenEvent <see cref="savedEvent"/> als ProjectionRequest für das Aggregat <see cref="aggregate"/> unter Nutzung von <see cref="aggregateEventMediatorRequestSender"/>.
    /// Die Veröffentlichung erfolgt nur, falls in  <see cref="eventStorePublishingModes"/> <see cref="EventStorePublishingModes.SendProjectionRequestUsingMediator"/> gesetzt ist./>
    /// </summary>
    ...
    /// <summary>
    /// Veröffentlicht das DomänenEvent <see cref="savedEvent"/> als EventOccured für das Aggregat <see cref="aggregate"/> unter Nutzung von <see cref="aggregateEventMediatorRequestSender"/>.
    /// Die Veröffentlichung erfolgt nur, falls in  <see cref="eventStorePublishingModes"/> <see cref="EventStorePublishingModes.SendEventOccuredRequestUsingMediator"/> gesetzt ist
    /// </summary>
    ...
    /// <summary>
    /// Veröffentlicht das DomänenEvent <see cref="savedEvent"/> als ProjectionRequest in Form einer Notification für das Aggregat <see cref="aggregate"/> unter Nutzung von <see cref="aggregateEventMediatorNotificationSender"/>
    /// Die Veröffentlichung erfolgt nur, falls in  <see cref="eventStorePublishingModes"/> <see cref="EventStorePublishingModes.NotifyForProjectionUsingMediator"/> gesetzt ist.
    /// </summary>
    public async Task PublishProjectionEventAsync<TAggregate>(IAggregateEvent savedEvent, TAggregate aggregate)
        where TAggregate : EventSourcingAggregateRoot
    {
        if ((this.eventStorePublishingModes &
             EventStorePublishingModes.NotifyForProjectionUsingMediator) != EventStorePublishingModes.None)
        {
    ...
    /// <summary>
    /// Veröffentlicht das DomänenEvent <see cref="savedEvent"/> als EventOccured für das Aggregat <see cref="aggregate"/> unter Nutzung von <see cref="aggregateEventMediatorNotificationSender"/>
    /// als Notification.
    /// Die Veröffentlichung erfolgt nur, falls in  <see cref="eventStorePublishingModes"/> <see cref="EventStorePublishingModes.NotifyEventOccuredUsingMediator"/> gesetzt ist.
    /// </summary>
    public async Task PublishEventOccuredAsync<TAggregate>(IAggregateEvent savedEvent, TAggregate aggregate)
        where TAggregate : EventSourcingAggregateRoot
    {
        if ((this.eventStorePublishingModes &
             EventStorePublishingModes.NotifyEventOccuredUsingMediator) != EventStorePublishingModes.None)
        {
