...
public class PulsarMessageBroker : MessageBrokerBase, IAsyncDisposable
{
        ...
        : base(options.LoggerFactory, options.HandlerFactory, options.Serializer, options.PublisherBehaviors, options.HandlerBehaviors)
    {
        ...
        {
            this.client = PulsarClient.Builder() // https://pulsar.apache.org/docs/en/client-libraries-dotnet/#create-client
        ...
        {
            throw new Exception($"Cannot create Pulsar client, {nameof(options.ServiceUrl)} option value must be supplied.");
        }
    ...
    protected override async Task OnPublish(Application.Messaging.IMessage message, CancellationToken cancellationToken)
    {
        var topic = message.GetType().PrettyName(false);

        var producer = this.producers.GetOrAdd(topic, this.client.NewProducer()
    ...
    protected override async Task OnSubscribe(Type messageType, Type handlerType)
    {
        ...
        this.Logger.LogDebug("{LogKey} pulsar consumer created (subscription={MessageSubscriptionName}, topic={MessageTopicName})", Constants.LogKey, consumer.SubscriptionName, consumer.Topic);
        ...
        this.Logger.LogDebug("{LogKey} pulsar consumer done (subscription={MessageSubscriptionName}, topic={MessageTopicName})", Constants.LogKey, consumer.SubscriptionName, consumer.Topic);
