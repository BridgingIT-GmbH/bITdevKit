...
using BridgingIT.DevKit.Common.UnitTests;
...
public class PropertyBackingFieldContractResolverTests
{
    ...
    public void CreateProperty_ObjectWithBackedCollection_DeserializesTheCollection()
    {
        ...
        entity.Locations.Count().ShouldBe(result.Locations.Count()); // make sure the locations collection (with private backing field)
        entity.Locations.Count().ShouldBe(3);
        entity.Locations.First().Name.ShouldBe(result.Locations.First().Name);
        entity.Locations.Last().Name.ShouldBe(result.Locations.Last().Name);
    }
}
