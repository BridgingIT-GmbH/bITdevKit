...
public abstract class EntityFrameworkDocumentStoreProviderTestsBase
{
    ...
    public virtual async Task FindAsync_WithDocumentKeyAndFilterFullMatch_ReturnsFilteredEntities()
    {
        ...
        var ticks = DateTime.UtcNow.Ticks;
        var sut = this.GetProvider();
        await sut.UpsertAsync(new DocumentKey("partition", "row" + ticks), new PersonStub { Id = Guid.NewGuid(), Nationality = "USA", FirstName = "Mary" + ticks, LastName = "Jane", Age = 18 });
        await sut.UpsertAsync(new DocumentKey("partition", "row" + ticks + "a"), new PersonStub { Id = Guid.NewGuid(), Nationality = "USA", FirstName = "John", LastName = "Doe", Age = 18 });
        await sut.UpsertAsync(new DocumentKey("partition", "row" + ticks + "b"), new PersonStub { Id = Guid.NewGuid(), Nationality = "USA", FirstName = "John", LastName = "Doe", Age = 18 });
        await sut.UpsertAsync(new DocumentKey("partition", "row" + ticks + "c"), new PersonStub { Id = Guid.NewGuid(), Nationality = "USA", FirstName = "John", LastName = "Doe", Age = 18 });
        await sut.UpsertAsync(new DocumentKey("partition", "row" + ticks + "d"), new PersonStub { Id = Guid.NewGuid(), Nationality = "USA", FirstName = "John", LastName = "Doe", Age = 18 });
