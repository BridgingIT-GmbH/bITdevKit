...
public class EventStoreTests
{
    ...
    public async Task EventStoreTest_Warmup()
    {
        ...
        await store.SaveEventsAsync(person, CancellationToken.None).AnyContext();
    }
    ...
    public async Task EventStoreTest_Events()
    {
        var mediator = Substitute.For<IMediator>();
        var eventStoreRep = Substitute.For<IEventStoreRepository>();
        eventStoreRep.When(x => x.ExecuteScopedAsync(Arg.Any<Func<Task>>())).Do(async x =>
         {
             var operation = x.Args().First() as Func<Task>;
             if (operation is not null)
             {
                 await operation.Invoke().AnyContext();
             }
         });
        var publishAggregateSender = Substitute.For<IPublishAggregateEventSender>();
        ...
        await store.SaveEventsAsync(person, CancellationToken.None).AnyContext();
        await publishAggregateSender.Received(1).WriteToOutboxAsync(evt, person).AnyContext();
        await publishAggregateSender.Received(1).SendEventOccuredAsync(evt, person).AnyContext();
        await publishAggregateSender.Received(1).PublishEventOccuredAsync(evt, person).AnyContext();
        await publishAggregateSender.Received(1).PublishProjectionEventAsync(evt, person).AnyContext();
        await publishAggregateSender.Received(1).SendProjectionEventAsync(evt, person).AnyContext();
        await eventStoreRep.Received(1).AddAsync<Person>(evt, CancellationToken.None).AnyContext();
    }
