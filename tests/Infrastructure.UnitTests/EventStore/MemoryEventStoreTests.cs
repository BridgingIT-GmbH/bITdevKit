...
public class MemoryEventStoreTests
{
    ...
    public async Task AddEventsToMemoryEventstore()
    {
        ...
        await store.AddAsync<Person>(personCreatedEvent, CancellationToken.None)
            .AnyContext();
        await store.AddAsync<Person>(new ChangeSurnameEvent(personCreatedEvent.AggregateId, 2, "GB"),
                CancellationToken.None)
            .AnyContext();
        var list = await store.GetEventsAsync<Person>(personCreatedEvent.AggregateId, CancellationToken.None)
            .AnyContext();
        personCreatedEvent.ToString()
            .ShouldNotBe(Guid.Empty.ToString());
    ...
    public async Task AddEventsToMemoryEventstoreGetIds()
    {
        ...
        await store.AddAsync<Person>(personCreatedEvent2, CancellationToken.None)
            .AnyContext();

        var orderEvent = new OrderCreatedEvent();
        await store.AddAsync<Order>(orderEvent, CancellationToken.None)
            .AnyContext();

        var ids = await store.GetAggregateIdsAsync(CancellationToken.None)
            .AnyContext();
        ids.Length.ShouldBe(3);
        ids.Contains(personCreatedEvent.AggregateId)
            .ShouldBeTrue();
        ids.Contains(personCreatedEvent2.AggregateId)
            .ShouldBeTrue();
        ids.Contains(orderEvent.AggregateId)
            .ShouldBeTrue();
        ids.Contains(Guid.NewGuid())
            .ShouldBeFalse();
    }
    ...
    {
        var aggregateRegistration = Substitute.For<IEventStoreAggregateRegistration>();
        aggregateRegistration.GetImmutableName<Person>()
            .Returns(PersonInfinityImmutableTypeIdentifierName);
        aggregateRegistration.GetImmutableName<Order>()
            .Returns(OrderInfinityImmutableTypeIdentifierName);
        var store = new InMemoryEventStoreRepository(this.eventSerializer, aggregateRegistration);
        var personCreatedEvent = new PersonCreatedEvent("Microsoft", "GovernanceBoard");
        await store.AddAsync<Person>(personCreatedEvent, CancellationToken.None)
            .AnyContext();
        ...
        var ids = await store.GetAggregateIdsAsync<Person>(CancellationToken.None)
            .AnyContext();
        ...
        ids = await store.GetAggregateIdsAsync<Order>(CancellationToken.None)
            .AnyContext();
